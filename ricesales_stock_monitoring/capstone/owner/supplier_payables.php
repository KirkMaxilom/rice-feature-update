          ...
          <?php if($payables && $payables->num_rows > 0): ?>
            <?php while($row = $payables->fetch_assoc()): ?>
              <?php
                ...
                $due = $row['due_date'] ?? '';
                $penalty = 0;  // No penalties for suppliers (customers only)
                $total_due = $balance;
                        ...
                        <form method="POST" class="row g-2 m-0">
                          ...
                          <div class="col-12 col-md-4">
                            ...
                            <input type="number" step="0.01" min="0.01"
                                   max="<?= htmlspecialchars((string)$total_due) ?>"
                                   name="amount" class="form-control form-control-sm" required>
                            <div class="small text-muted">
                              Balance: ₱<?= number_format($balance,2) ?>
                              <?php if($penalty > 0): ?>
                                <br>Penalty: ₱<?= number_format($penalty,2) ?>
                                <br><strong>Total Due: ₱<?= number_format($total_due,2) ?></strong>
                              <?php endif; ?>
                            </div>
                          </div>
